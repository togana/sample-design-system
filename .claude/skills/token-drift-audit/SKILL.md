...
- `src/semantic-tokens/colors.ts`
- その他カテゴリのセマンティックトークンファイル（存在する場合）

**登録先:**

- `src/tokens/index.ts`
- `src/semantic-tokens/index.ts`
- `panda.config.ts`
...
`src/semantic-tokens/` で定義されているが、どのレシピ・コンポーネントからも参照されていないセマンティックトークン。
...
**検出方法:** プリミティブトークンのキーパス（例: `colors.blue.600`）を生成し、`src/semantic-tokens/` と `panda.config.ts` 内で `{colors.blue.600}` の形式を検索する。
...
`panda.config.ts` の `semanticTokens` に直接記述されているが、専用ファイル（`src/semantic-tokens/*.ts`）に切り出されていないトークン。

**検出方法:** `panda.config.ts` の `semanticTokens` ブロック内で、`src/semantic-tokens/` からインポートした変数ではなくオブジェクトリテラルで直接定義されているエントリを検出する。
...
**検出方法:** `src/semantic-tokens/colors.ts` の各トークンの `value` が `{ base, _dark }` 形式になっているか確認する。`white` / `black` 等のリテラル値で `base` と `_dark` が同値の場合は許容する。

#### D6: エクスポート/登録の不整合（critical）

- `src/tokens/*.ts` でエクスポートしているが `src/tokens/index.ts` に含まれていない
- `src/semantic-tokens/*.ts` でエクスポートしているが `src/semantic-tokens/index.ts` に含まれていない
...
- **[D1]** `src/semantic-tokens/colors.ts` — `icon.success` はどのコンポーネントからも参照されていない
- **[D4]** `src/semantic-tokens/colors.ts:28` — `{colors.purple.600}` を参照しているが `colors.ts` に `purple` は未定義
- ...

### Warning

- **[D3]** `panda.config.ts:52` — `fontSizes.body` がインラインで定義。`src/semantic-tokens/` への切り出しを推奨
- **[D5]** `src/semantic-tokens/colors.ts:76` — `text.onFill` の `base` と `_dark` が同値（意図的か確認）
- ...
